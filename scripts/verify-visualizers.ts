...
for (const entry of entries) {
  ...
  const hasDistinctFrameContent = frames.length === 4 && new Set(frames.map((item) => item.commentary)).size === 4 && frames.every((item) => item.state.some((state) => state.label === 'phase'));
  if (!entry.problemId || !entry.lessonPath || !entry.hasVisualizer || !hasCustomVisualizerLesson(entry.lessonPath) || !hasProblemSpecificScenario || !hasDistinctFrameContent || frames.length < 4 || !profile.focus || !profile.input || !profile.output || profile.code.length < 5 || !hasState || !hasValidCodeLines) {
...
console.log(`frames: ${entries.length ? 'every mapped lesson has 4 distinct steps, input/output state, and valid highlighted code' : 'none'}`);
