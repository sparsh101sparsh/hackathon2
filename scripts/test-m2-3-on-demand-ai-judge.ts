...
import { POST as aiJudgePost } from '../app/api/ai/judge/route';
...
async function runEmpiricalTests() {
  console.log('================================================================');
  console.log('  EMPIRICAL CHALLENGER: MILESTONE 2 & 3 (ON-DEMAND & AI JUDGE) ');
  console.log('================================================================\n');

  // --- SECTION 1: AI JUDGE API (/api/ai/judge) ---
  ...
  try {
    ...
    const res = await aiJudgePost(req);
    ...
    record('AI Judge', 'Time Complexity O(N^2) Pattern Match', false, `Error: ${err.message}`);
  }
  ...
  try {
    ...
    record('AI Judge', 'Time Complexity O(N log N) Pattern Match', false, `Error: ${err.message}`);
  }
  ...
  try {
    ...
    record('AI Judge', 'Time Complexity O(1) Pattern Match', false, `Error: ${err.message}`);
  }
  ...
  try {
    ...
    record('AI Judge', 'Time Complexity O(log N) Binary Search', false, `Error: ${err.message}`);
  }
  ...
  try {
    ...
    const resGuards = await aiJudgePost(reqGuards);
    ...
    const resNoGuards = await aiJudgePost(reqNoGuards);
    ...
    record('AI Judge', 'Edge Case Resilience Score Differentiation', false, `Error: ${err.message}`);
  }
  ...
  try {
    ...
    record('AI Judge', 'Graceful Handling of Syntax Errors', false, `Error: ${err.message}`);
  }
  ...
  try {
    ...
    const resWrong = await aiJudgePost(reqWrong);
    ...
    record('AI Judge', 'Test Case Results Integration', false, `Error: ${err.message}`);
  }

  // 1.8 Empty Body Request
  try {
    const reqEmpty = createJsonRequest('http://localhost:3000/api/ai/judge', {});
    const resEmpty = await aiJudgePost(reqEmpty);
    ...
    record('AI Judge', 'Empty Request Body Handling', false, `Error: ${err.message}`);
  }
  ...
  // 2.4 AI Bot Participant Joining (`addAiBot: true` / `isAiDuel: true`)
  try {
    const req = createJsonRequest('http://localhost:3000/api/rooms/create', {
      name: '1v1 vs Grandmaster Gemini',
      difficulty: 'MIXED',
      mode: 'DUEL',
      addAiBot: true,
      isAiDuel: true,
    ...
    const aiBot = participants.find((p: any) => p.userId === 'ai_bot_gemini');
    ...
    const passed =
      ...
      userPart !== undefined &&
      aiBot !== undefined &&
      aiBot.userName === 'Grandmaster Gemini';

    record(
      'Rooms Create',
      'AI Bot Participant Joining (Grandmaster Gemini attached)',
      passed,
      `Participants count: ${participants.length}, Bot name: "${aiBot?.userName}", Bot ID: "${aiBot?.userId}"`
    );
  } catch (err: any) {
    record('Rooms Create', 'AI Bot Participant Joining', false, `Error: ${err.message}`);
  }

  // 2.5 ADVERSARIAL CHALLENGE: Passing mode: 'AI_DUEL' without addAiBot / isAiDuel flags
  try {
    const req = createJsonRequest('http://localhost:3000/api/rooms/create', {
      name: 'AI Duel Challenge',
      difficulty: 'MIXED',
      mode: 'AI_DUEL',
    }, token);
    const res = await createRoomPost(req);
    ...
    const isBotAdded = aiBot !== undefined;
    record(
      'Adversarial Challenge',
      "Passing mode: 'AI_DUEL' attaches AI Bot participant",
      isBotAdded,
      `Participants count: ${participants.length}, AI Bot attached: ${isBotAdded}`
    );
  } catch (err: any) {
  ...
  try {
    ...
    const getRes = await getRoomByCode(getReq, { params: { code: roomCode } });
